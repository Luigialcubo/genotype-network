...
void derivada_extra(double rho[],double mu[],double X_uni[],double A_uni[],ParametrosSIMS p,double drho[],double dmu[], double betas[]){
    ...
    */
    ...
    for(i=0;i<p.N;i++){
        ...
        drho[i]=betas[i]*rho[i]*S-mu[i]*rho[i]-p.Dx*mutacion;
        dmu[i]=p.alfa*inmunidad-p.gamma*(mu[i]-p.mu_0);
    }
};
...
void paso_rk4_sims_extra(double rho[],double mu[],double X_uni[],double A_uni[],ParametrosSIMS p, double betas[]){
    ...
    // Primero calculo el k1,k2,k3 y k4 para rho y mu de cada cepa
    //k1
    derivada_extra(rho,mu,X_uni,A_uni,p,k1_rho,k1_mu, betas);
    ...
    derivada_extra(rho_aux,mu_aux,X_uni,A_uni,p,k2_rho,k2_mu, betas);
    ...
    derivada_extra(rho_aux,mu_aux,X_uni,A_uni,p,k3_rho,k3_mu, betas);
    ...
    derivada_extra(rho_aux,mu_aux,X_uni,A_uni,p,k4_rho,k4_mu, betas);
