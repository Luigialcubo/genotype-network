...
int main(){
   ...
   /* //Retícula 3x70
    ...
    //Heterogénea
    double *A_uni_het = malloc(sizeof(double) * pa_het_gamma_003.N * pa_het_gamma_003.N);
    double *X_uni_het = malloc(sizeof(double) * pa_het_gamma_003.N * pa_het_gamma_003.N);
    ...
    int i;
    double I_het_gamma_003 = 0.0, t_het_gamma_003 = 0.0;
    int iter_het_gamma_003 = 0;
    
    for(i=0; i<pa_het_gamma_003.N; i++){
        double probabilidad = (double)rand() / RAND_MAX;

        if (probabilidad < 0.25) {
            // GRUPO 1: 25% de la población
            // Confinamiento voluntario (reduce drásticamente sus contactos)
            beta_aleatoria[i] = pa_het_gamma_003.beta * 0.40;
            
        } else if (probabilidad < 0.65 && probabilidad >= 0.25) {
            // GRUPO 2: 40% de la población (los que caen entre 0.30 y 0.80)
            // Solo mascarilla (reduce la infectividad como vimos antes)
            beta_aleatoria[i] = pa_het_gamma_003.beta * 0.90;
            
        } else {
            // GRUPO 3: 20% de la población restante (los que caen entre 0.80 y 1.0)
            // No cumplen nada (mantienen la infectividad al 100%)
            beta_aleatoria[i] = pa_het_gamma_003.beta;
        }
    }
    FILE *sims_het_gamma_0;
    FILE *sims_het_gamma_003_extra2;
    ...
    sims_het_gamma_003_extra2 = fopen("results/fig3/sims_het_gamma_003_extra2.txt", "w");
    ...
    if (sims_het_gamma_0 == NULL || sims_het_gamma_003_extra2 == NULL || sims_hom_gamma_0 == NULL || sims_hom_gamma_003 == NULL || sims_reticula_gamma_0 == NULL || sims_reticula_gamma_003 == NULL) {
...
*/
    ...
    do{
        //if (t_het_gamma_003 < 100*dT){
         //   fprintf(sims_het_gamma_003_extra2, "%f\t%f\n", t_het_gamma_003, I_het_gamma_003);
         //   I_het_gamma_003 = 0;
      ...
      //  }else{
            ...
            fprintf(sims_het_gamma_003_extra2, "%f\t%f\n", t_het_gamma_003, I_het_gamma_003);
            I_het_gamma_003 = 0;
            paso_rk4_sims_extra(rho_het_gamma_003,mu_het_gamma_003,X_uni_het,A_uni_het,pa_het_gamma_003, beta_aleatoria);
            for(i=0;i<pa_het_gamma_003.N;i++){
                I_het_gamma_003+=rho_het_gamma_003[i];
            }
...
*/
    ...
    fclose(sims_het_gamma_003_extra2);
